...
class GenSumo:
    ...
    def __generate_flow(self):
        ...
        time_off_data = random.choices(time_pool_off, k=num_route * int(num_veh_off * self.mpr_op))  # CAV off-peak
        time_peak_hv = random.choices(time_pool_peak, k=num_route * int(num_veh_peak * (1 - self.mpr_peak)))  # HV peak
        time_peak_data = random.choices(time_pool_peak, k=num_route * int(num_veh_peak * self.mpr_peak))  # CAV peak
    ...
    def __generate_sim(self):
        ...
        <output>
            <summary value="%s"/>
            <tripinfo value="%s"/>
        </output>""" % ('summary.out.xml', 'tripinfo.out.xml'), file=cfg)
            print("""
        <time>
            <begin value="0"/>
        </time>
